...
public class ShopScreen extends Screen {
	...
	private void draw() {
		 ...
		 * drawManager.drawShopModal(this, Item.itemregistry_ship.get(invrow).name,
		 * String.valueOf(Item.itemregistry_ship.get(invrow).price),
		 * shopmodaltype.SM_YESNO, modaloption);
		 * }
		 * if (invcol == 1) {
		 * if (invrow == 0 || invrow == 1 || invrow == 2)
		 * drawManager.drawShopModal(this, Item.itemregistry_bgm.get(invrow).name,
		 * String.valueOf(Item.itemregistry_bgm.get(invrow).price),
		 * shopmodaltype.SM_YESNO, modaloption);
		 * }
		 * }
		 * 
